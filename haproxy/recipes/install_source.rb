...
remote_file "#{Chef::Config[:file_cache_path]}/pcre-8.33.tar.bz2" do
  source "http://downloads.sourceforge.net/project/pcre/pcre/8.33/pcre-8.33.tar.bz2"
  action :create_if_missing
end

bash "compile_pcre" do
  cwd Chef::Config[:file_cache_path]
  code <<-EOH
    tar xf pcre-8.33.tar.bz2
    cd pcre-8.33
    ./configure --prefix=#{node['haproxy']['source']['prefix']} --libdir=/usr/local/lib64 --enable-jit --enable-utf
    make && make install
    echo /usr/local/lib64 >/etc/ld.so.conf.d/pcre-8.33.x86_64.conf
  EOH
...
make_cmd << " USE_STATIC_PCRE=1 USE_PCRE_JIT=1" if node['haproxy']['source']['use_pcre']
make_cmd << " USE_OPENSSL=1" if node['haproxy']['source']['use_openssl']
make_cmd << " USE_ZLIB=1 ADDLIB=-lz" if node['haproxy']['source']['use_zlib']
